...
| :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| **Phase 6.4** | Localhost desktop boundary remediation | `a0a008d` | `a383404` | PASS | YES | NO | `~/aura_mail_ai_phase6_4_artifacts/` | Enforced loopback peer validation & CORS |
| **Phase 6.5** | Browser context & Fetch Metadata checks | `1f93ebf` | `a0a008d` | PASS | YES | NO | `~/aura_mail_ai_phase6_5_artifacts/` | Sec-Fetch-Site and Origin filtering |
| **Phase 7** | Multi-account isolation candidate | `c35a6fa` | `1f93ebf` | FAIL | NO | YES | `~/aura_mail_ai_phase7_artifacts/` | Test runner concurrency issue |
| **Phase 7.1** | Multi-account isolation remediation | `b2d0529` | `c35a6fa` | PASS | YES | NO | `~/aura_mail_ai_phase7_1_artifacts/` | Multi-account state fencing complete |
| **Phase 8** | Outlook cloud-draft staging | `b3314ec` | `b2d0529` | PASS | YES | NO | `~/aura_mail_ai_phase8_artifacts/` | 52 JS tests for draft staging without send |
...
| **Phase 11.1 (p)**| Documentation truth remediation | `29f13c3` | `0b9720a` | FAIL (proc) | NO | YES | `~/aura_mail_ai_phase11_1_artifacts/` | Substantive PASS, procedural commit subject fail |
| **Phase 11.1 (c)**| Documentation truth corrected | `cfa15a3` | `0b9720a` | PASS | YES | NO | `~/aura_mail_ai_phase11_1_artifacts/` | Frozen documentation baseline |
| **Phase 12** | Independent adversarial regression audit | `cfa15a3` | `0b9720a` | CAUTION | YES | NO | `evidence/security-remediation-v1.1/phase-12/` | Read-only audit; identified finding P12-001 |
...
All future development, maintenance, and auditing within Aura Mail AI must adhere to the following thirteen governing principles:
...
11. **Protocol revisions are retained as historical governance evidence:** All revisions of governing remediation protocols are permanently archived.
12. **Peer-review evidence is preserved independently from implementation evidence:** External peer reviews remain distinct artifacts in the archive.
13. **Unknown provenance is explicitly labeled UNKNOWN rather than inferred:** Incomplete historical metadata must not be fabricated.
